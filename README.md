...
BloodyGround는 언리얼 엔진 5로 만든 3인칭 멀티플레이 슈팅 프로젝트입니다. "빠른 조준 감각"과 "신뢰할 수 있는 동기화"를 목표로 캐릭터, 전투, AI, 세션 플로우를 컴포넌트화했습니다. 이 문서는 프로젝트 구조를 처음 접하는 개발자도 곧바로 흐름을 파악할 수 있도록 실제 코드에서 발췌한 스니펫과 함께 핵심 모듈을 설명합니다.

## 한눈에 보기
- **엔진 & 타겟:** Unreal Engine 5 · 멀티플레이 TPS 프로토타입
- **핵심 철학:** 기능은 컴포넌트/서브시스템으로 분리, 네트워크 판정은 항상 서버 권위 기반, HUD는 상태와 즉시 동기화
...
- **네트워크 디버깅**: PIE 멀티 클라이언트를 띄워 RTT 표시(PlayerController)와 히트 검증 로그(Weapon/ServerLocation)를 동시에 확인하면 재현이 쉽습니다.
- **기여 시 체크리스트**
  1. 새 컴포넌트는 Replication 여부를 명확히 선언합니다.
  2. HUD와 연동되는 데이터는 `AInGameHUD` → `UInGameWidget` 체인을 통해 갱신합니다.
  3. 세션 관련 코드는 `UMultiplayerSessionsSubsystem` 이벤트 델리게이트를 활용해 UI와 상태를 동기화합니다.

앞으로도 동일한 설계 철학을 유지하며 시스템을 확장할 예정입니다. 특정 모듈을 더 깊이 분석하려면 위 스니펫 경로를 그대로 따라가면 됩니다.
